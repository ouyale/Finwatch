...
The `CustomerPreprocessor` class in `finwatch/preprocessor.py` handles all data cleaning. It is a fitted sklearn transformer - it learns statistics from training data during `.fit()`, saves them, and applies those exact saved values at scoring time via `.transform()`. This prevents leakage from inference data into the preprocessing statistics.
...
Raw columns often carry less signal than their relationships. `finwatch/features.py` creates derived features in three areas.

Financial stress ratios capture the relationship between credit obligation and financial capacity:
...
- `ext_source_mean / min / std` - aggregates of the three credit bureau scores, treating -1 sentinel as missing.

Employment stability: `days_employed_pct_of_age` - what proportion of your life have you been employed? A 50-year-old with 2 years of employment tells a different story than a 25-year-old in the same position.
...
Every individual scoring decision is accompanied by the top contributing SHAP features and their direction of influence, satisfying **UK GDPR Article 22**.
...
Retraining is triggered in three cases: on a monthly schedule, when PSI exceeds 0.20 on any key feature, and when any ONS indicator moves more than 1.5 standard deviations from its training baseline.
